...
const MusicApp = () => {
  ...
  // Σωστό Audio Object Initialization
  ...
  useEffect(() => {
    ...
    // Αρχική φόρτωση για να μην είναι άδεια η οθόνη
    ...
    <div className="flex h-screen bg-[#020205] text-white overflow-hidden font-sans text-sm">
      ...
      <main className="flex-1 flex flex-col bg-[#020205] relative pb-24">
        <header className="p-4 bg-black/20 backdrop-blur-md border-b border-white/5 flex items-center justify-between z-50">
          ...
          <button className="bg-indigo-600 px-6 py-2 rounded-lg font-bold uppercase text-[10px] tracking-widest ml-4">Sign Up</button>
        </header>
        ...
        {playingTrack && (
          <div className="fixed bottom-0 left-0 right-0 bg-[#080810]/95 backdrop-blur-2xl border-t border-white/10 px-6 py-4 flex items-center gap-6 z-[100]">
            <div className="flex items-center gap-4 w-64 shrink-0">
              <img src={playingTrack.album?.cover_medium || playingTrack.albumArt} className="w-12 h-12 rounded-lg" alt="" />
              <div className="truncate"><div className="font-bold text-sm truncate">{playingTrack.title}</div></div>
            </div>
            <div className="flex-1 max-w-xl mx-auto flex flex-col items-center gap-2">
