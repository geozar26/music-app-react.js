          ...
          <div className="w-[450px] relative" ref={searchRef}>
            ...
            {showHistory && searchHistory.length > 0 && (
              <div className="absolute top-[110%] left-0 w-full bg-[#0a0a0a] border border-white/10 rounded-2xl p-1 shadow-2xl z-[150] backdrop-blur-xl">
                {searchHistory
                  .filter(item => item.toLowerCase().startsWith(searchQuery.toLowerCase()))
                  .map((h, i) => (
                    <div key={i} className="flex items-center justify-between group hover:bg-white/5 rounded-xl transition-all cursor-pointer">
                      ...
                      <button 
                        ...
                        className="mr-2 p-1 text-zinc-600 hover:text-red-500 transition-colors"
                      >
                        <X size={14} />
                      </button>
                    </div>
        ...
        <div className="p-8">
          <div className="flex items-center gap-6 mb-10">
            ...
            {view === 'library' && favorites.length > 0 && (
              <button onClick={() => {setFavorites([]); localStorage.removeItem('beatstream_favs');}} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 hover:bg-red-500/10 hover:border-red-500/50 transition-all group mt-2 ml-10 shadow-lg">
          ...
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[850px] bg-black/90 backdrop-blur-2xl border border-white/10 p-4 rounded-[2.5rem] flex items-center justify-between shadow-2xl z-[200]">
            <div className="flex items-center gap-4 w-[30%]">
              ...
              <div className="truncate">
                <h4 className="text-[12px] font-bold truncate text-white">{playingTrack.title}</h4>
