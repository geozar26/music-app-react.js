...
const MusicApp = () => {
  ...
  // Λήψη αγαπημένων από τη Dexie
  const favoriteTracks = useLiveQuery(() => {
    return db?.tracks ? db.tracks.toArray() : [];
  }, []) || [];

  // Λήψη ιστορικού
  const searchHistory = useLiveQuery(() => {
    return db?.history ? db.history.reverse().toArray() : [];
  }, []) || [];

  // Φιλτράρισμα ιστορικού βάσει αυτού που γράφει ο χρήστης
  const filteredHistory = searchQuery.trim() === '' 
    ? searchHistory.slice(0, 6) // Αν είναι κενό, δείξε τα τελευταία 6
                ...
                <div key={track.id} className="bg-[#111111]/40 p-4 rounded-[2rem] border border-white/5 relative group">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    ...
                    <button onClick={(e) => toggleLike(e, track)}>
                      {/* Η καρδιά γίνεται κόκκινη και γεμίζει αν το τραγούδι είναι liked */}
                      <Heart 
                        size={18} 
