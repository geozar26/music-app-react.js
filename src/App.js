      ...
      <main className="flex-1 flex flex-col bg-[#020205] relative pb-24">
        ...
        <header className="p-4 flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-8 pr-4">
            <button className="text-[11px] font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors">
              Install
            </button>
            <button className="text-[11px] font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors">
              Log In
            </button>
            <button className="bg-[#6366f1] hover:bg-[#5558e3] px-9 py-2.5 rounded-xl font-bold uppercase text-[11px] tracking-widest shadow-lg shadow-indigo-500/20 transition-all">
              Sign Up
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8" onClick={() => setActiveMenu(null)}>
          <h2 className="text-[40px] font-black uppercase italic mb-10 text-zinc-400 tracking-tighter">Discover</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {tracks.map((track) => (
              <div key={track.id} className="bg-[#111111]/40 p-4 rounded-[2rem] group border border-white/5 relative">
                ...
                <p className="text-[11px] text-zinc-600 truncate px-1 mb-4">{track.artist?.name}</p>
