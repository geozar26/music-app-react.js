      ...
      <aside className="w-64 bg-[#080810] border-r border-white/5 flex flex-col shrink-0">
        <div className="p-5 flex flex-col h-full">
          ...
          <button onClick={() => setView('library')} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'library' ? 'bg-indigo-500/15 text-indigo-400' : 'text-zinc-500 hover:text-white'}`}>
            <LibraryIcon size={20} /> <span className="font-black uppercase tracking-widest text-[11px]">Library</span>
          </button>
        </div>
      </aside>
            ...
            {(view === 'discover' ? tracks : favorites).map((track) => (
              <div key={track.id} className="bg-white/[0.03] p-4 rounded-2xl group border border-white/5 relative hover:bg-white/[0.07] transition-all">
                <div className="relative mb-4 aspect-square overflow-hidden rounded-xl">
                  ...
                  <button onClick={() => handlePlay(track)} className="absolute inset-0 m-auto w-12 h-12 bg-indigo-500 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all shadow-xl">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => { if (db.favorites) { favorites.some(f => f.id === track.id) ? db.favorites.delete(track.id) : db.favorites.add({ id: track.id, title: track.title, artist: track.artist?.name || track.artist, albumArt: track.album?.cover_medium || track.albumArt, preview: track.preview }); }}} className="p-1">
            ...
            <div className="flex-1 flex flex-col items-center gap-2 max-w-2xl mx-auto">
              <div className="flex items-center gap-3 w-full">
                ...
                <div className="flex-1 h-1.5 bg-white/10 rounded-full relative overflow-hidden">
                  <div className="absolute top-0 left-0 h-full bg-indigo-500" style={{ width: `${(currentTime / duration) * 100}%` }} />
                </div>
                <span className="text-[10px] font-mono text-zinc-500">{formatTime(duration)}</span>
              </div>
            </div>
            <div className="w-72 flex justify-end">
               <button onClick={() => handlePlay(playingTrack)} className="w-10 h-10 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 transition-all"><Pause size={20} fill="black" /></button>
            </div>
          </div>
        )}
      </main>
