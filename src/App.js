...
const MusicApp = () => {
  ...
  const toggleLike = async (e, track) => {
    e.stopPropagation();
    if (!db?.tracks) return; // Αν η βάση δεν είναι έτοιμη, σταμάτα
  ...
  return (
    <div className="flex h-screen bg-[#020205] text-white overflow-hidden font-sans text-sm">
      <aside className="w-64 bg-black flex flex-col shrink-0 p-6">
        ...
        <button onClick={() => setView('library')} className={`flex items-center gap-3 px-2 transition-colors ${view === 'library' ? 'text-white' : 'text-zinc-500 hover:text-white'}`}>
          <LibraryIcon size={18} />
              ...
              return (
                <div key={track.id} className="bg-[#111111]/40 p-4 rounded-[2rem] group border border-white/5 relative">
                  ...
                  <div className="flex justify-between items-center px-1">
                    <button className="text-zinc-800 hover:text-white"><MoreVertical size={16} /></button>
