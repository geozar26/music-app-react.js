...
const MusicApp = () => {
  ...
  return (
    <div className="flex h-screen bg-[#020205] text-white overflow-hidden font-sans text-sm relative">
      ...
      <main className="flex-1 flex flex-col bg-gradient-to-br from-[#0d0d1a] to-[#020205] relative">
        <header className="p-4 flex items-center justify-between bg-black/20 backdrop-blur-xl border-b border-white/5 z-50">
                ...
                <div className="p-2">
                  {searchHistory.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between px-4 py-2 hover:bg-white/5 rounded-xl cursor-pointer group" onClick={() => { setSearchQuery(item.query); searchTracks(null, item.query); }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); db.searches.delete(item.id); }} className="opacity-0 group-hover:opacity-100 text-zinc-500 hover:text-indigo-400 transition">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
          ...
          <div className="flex items-center gap-10 ml-6 mr-12 shrink-0">
            ...
            <button className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-6 py-2 rounded-lg font-black uppercase text-[10px] tracking-widest shadow-lg hover:scale-105 transition-all">Sign Up</button>
          </div>
        </header>
