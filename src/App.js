...
const MusicApp = () => {
  ...
  // Κλείσιμο ιστορικού αν κάνεις κλικ έξω
  useEffect(() => {
  ...
  const handleSearch = async (e, queryOverride) => {
    ...
    try {
      const res = await axios.get(`https://deezerdevs-deezer.p.rapidapi.com/search?q=${finalQuery}`, {
        headers: {
          'x-rapidapi-key': '84e121a50dmsh4650b0d1f6e44fep1ebe78jsn56932706b2b1',
          'x-rapidapi-host': 'deezerdevs-deezer.p.rapidapi.com'
        }
      ...
      <main className="flex-1 flex flex-col relative pb-40">
        <header className="p-4 flex items-center justify-between z-[100] bg-[#020205]/80 backdrop-blur-md sticky top-0">
          <div className="w-[450px] relative" ref={searchRef}>
            ...
            {showHistory && searchHistory.length > 0 && (
              <div className="absolute top-[110%] left-0 w-full bg-[#0a0a0a] border border-white/10 rounded-2xl p-2 shadow-2xl backdrop-blur-xl z-[150]">
                <div className="flex items-center justify-between px-3 py-2 mb-1">
                  <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Recent Searches</span>
                  <button onClick={clearHistory} className="text-[10px] font-bold text-zinc-600 hover:text-red-500 transition-colors">Clear</button>
                </div>
                {searchHistory.map((item, idx) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-zinc-300 group-hover:text-white truncate">{item}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="p-8">
          <div className="flex items-center gap-6 mb-10">
            {view === 'library' && (
              <button onClick={() => setView('discover')} className="p-2 bg-white/5 rounded-full hover:bg-white/10 text-[#6366f1] transition-all">
            ...
            {view === 'library' && favorites.length > 0 && (
              <button onClick={() => {setFavorites([]); localStorage.removeItem('beatstream_favs');}} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 hover:bg-red-500/10 hover:border-red-500/50 transition-all group mt-2">
          ...
          {!isLoading && (view === 'library' ? favorites : tracks).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-zinc-600">
              <div className="bg-white/5 p-8 rounded-[3rem] mb-6"><Music3 size={64} className="opacity-20 text-[#6366f1]" /></div>
              <h3 className="text-xl font-bold text-white mb-2">No tracks found</h3>
              <p className="text-sm">Start adding some music to your collection!</p>
            </div>
          ) : (
        ...
        {playingTrack && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[850px] bg-black/90 backdrop-blur-2xl border border-white/10 p-4 rounded-[2.5rem] flex items-center justify-between shadow-2xl z-[200]">
            ...
            <div className="flex-1 flex flex-col items-center gap-2 max-w-[40%]">
              <button onClick={() => isPaused ? audioRef.current.play() : audioRef.current.pause()} className="bg-white p-2 rounded-full hover:scale-110 transition-transform shadow-lg">
