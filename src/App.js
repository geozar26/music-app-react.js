    ...
    <div className="flex h-screen bg-[#020205] text-white overflow-hidden font-sans text-sm relative">
      ...
      <main className="flex-1 flex flex-col bg-gradient-to-br from-[#0d0d1a] to-[#020205] relative pb-24">
        ...
        <header className="p-4 flex items-center justify-between bg-black/20 backdrop-blur-xl border-b border-white/5 z-50">
          <div className="flex-1 max-w-lg relative">
            <form onSubmit={searchTracks} className="relative">
              ...
              <input 
                ...
                placeholder="Search songs, artists..." value={searchQuery} onFocus={() => setShowHistory(true)} onChange={(e) => setSearchQuery(e.target.value)}
              />
            </form>
          ...
          <button className="bg-indigo-600 px-6 py-2 rounded-lg font-black uppercase text-[10px] tracking-widest shadow-lg hover:bg-indigo-500 transition-all">Sign Up</button>
        </header>
        ...
        {playingTrack && (
          <div className="fixed bottom-0 left-0 right-0 bg-[#080810]/95 backdrop-blur-2xl border-t border-white/10 px-6 py-4 z-[100] flex items-center gap-6">
            <div className="flex items-center gap-4 w-72 shrink-0">
              <img src={playingTrack.album?.cover_medium || playingTrack.albumArt} className="w-12 h-12 rounded-lg shadow-lg" alt="" />
              <div className="truncate">
                <div className="font-bold text-sm truncate text-white">{playingTrack.title}</div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-tighter">{playingTrack.artist?.name || playingTrack.artist}</div>
              </div>
            </div>
